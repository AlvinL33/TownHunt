...
class AccountInfoPageViewController: UIViewController, ModalTransitionListener {
    ...
    private func getDBAccountStats(){
        ...
        if dbInteraction.connectedToNetwork(){
            let responseJSON = DatabaseInteraction().postToDatabase(apiName: "getAccountStats.php", postData: "userID=\(self.userID)"){ (dbResponse: NSDictionary) in
